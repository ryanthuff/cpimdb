...
<div style="text-align: center; padding: 20px; border: 1px solid #000000; margin-bottom: 10px; margin-top: 10px;">Real Time Feed of instant messages in the enterprise</div>    
      <?php
        ...
        $query = "SELECT jm.to_jid,jm.from_jid,jm.body_string,jm.sent_date FROM jm ORDER BY jm.sent_date DESC limit 50";
        ...
        while ($line = pg_fetch_array($result, null, PGSQL_ASSOC)) {
              ...
              //HIDE CHAT ROOM COMMUNICATIONS IN LIVE STREAM
              $crIdent = array('NAME-OF-CHAT-ROOM','NAME-OF-CHAT-ROOM');
              if (!in_array($toParts_1[0], $crIdent) && !in_array($fromParts_1[0], $crIdent)) {
                  if ($line['body_string'] != '' OR !empty($line['body_string'])) {                    
